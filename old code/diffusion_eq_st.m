...
g.nx = 50;                           % Number of steps in space(x)
g.ny = 50;                           % Number of steps in space(y)
...
p.vis = 0.1;                         % Diffusion coefficient/viscocity
p.gamma = 0.01;                      % decay rate
p.production_rate = 0.1;             % rates at which bacteria produces AI
...
function D = coeff_matrix(g,p,BC_type)
    ...
    D=speye((g.nx-2)*(g.ny-2))-p.vis*p.dt*A;
end

function u = diffuse(u,D,bc,g,p,BC_type,edge)
    
    un=u;
%     un(sour~=0) = un(sour~=0) + sour(sour~=0);
%     plot_field(g.x,g.y,u)
...
%     shading interp
    ...
    axis ([0 2 0 2 0 2])
    view(2)
    title({['2-D Diffusion with {\nu} = ',num2str(p.vis)];['time (\itt) = ',num2str(t*p.dt)]})
...
%     map = brewermap(100,maps{i});
    ...
    hold on
    end%plotting the field variable
%     shading interp
%     colormap(gca,map)
    axis ([0 2 0 2 0 2])
    view(2)
    ...
    xlabel('X \rightarrow')
    ylabel('{\leftarrow} Y')
    zlabel('QS AI concentration (u) \rightarrow')
    drawnow; 
